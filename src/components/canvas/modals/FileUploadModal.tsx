...
export function FileUploadModal({
  ...
  const handleUrlSubmit = async (e: React.FormEvent) => {
    ...
    setIsUploading(true)
    try {
      const response = await fetch("/api/files/from-url", {
        method: "POST",
      ...
      if (!response.ok) {
        const error = await response.json().catch(() => ({}))
        throw new Error(error.error || error.message || "Failed to import from URL")
      }
          ...
          {uploadMethod === "file" ? (
            <>
              <div
                ...
                onDrop={handleDrop}
                onMouseEnter={(e) => {
                  if (!isDragging) e.currentTarget.style.borderColor = colors.gray
                }}
                onMouseLeave={(e) => {
                  if (!isDragging) e.currentTarget.style.borderColor = colors.gray + "80"
                }}
              >
                <input
                  ref={fileInputRef}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <button
                      ...
                      onMouseEnter={(e) => {
                        if (!isUploading) e.currentTarget.style.backgroundColor = colors.blueDark
                      }}
                      onMouseLeave={(e) => {
                        if (!isUploading) e.currentTarget.style.backgroundColor = colors.blue
                      }}
                    >
            ...
            <div className="space-y-4">
              <form onSubmit={handleUrlSubmit}>
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Paste a Google Docs / Sheets / Slides link"
              ...
              <p className="text-sm text-gray-500 text-center">
                Upload the link, edit in Google, then press <b>Sync</b> on the node to refresh!
              </p>
            </div>
          )}
        </div>
